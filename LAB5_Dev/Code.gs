/**
 * Web App for Lab 5: Cloud MQTT & Cross-Platform Dashboard
...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('ใบงานที่ 5: การสื่อสารผ่านระบบคลาวด์และแดชบอร์ดข้ามแพลตฟอร์ม (Cloud MQTT)')
...
function gradeSubmission(data) {
  var blankKeywords = ["publish|mqttClient.publish", "subscribe|mqttClient.subscribe"];
  var challengeKeywords = [
    "PubSubClient|mqttClient|mqtt",
    "tempThreshold|threshold|30|analog|potentiometer|soil|knob",
    "publish|esp-node/state|pubTopic",
    "subscribe|callback|esp-node/control/cmd|debounce|toggleCount"
  ];
  var q1Keywords = ["broker|ตัวกลาง","publish|subscribe|กระจาย","ข้ามเครือข่าย|อินเทอร์เน็ต|nat|firewall","หลายเครื่อง|decouple|เบา"];
  var q2Keywords = ["qos|quality of service","retained|บันทึกล่าสุด","last will|lwt|พินัยกรรม","keep-alive|ping"];
...
function submitLabData(data) {
  try {
    ...
    var sheetName = "Lab 5 Submissions";
    ...
    if (!sheet) {
      ...
      var headers = [
        ...
        "คำถามข้อที่ 1 (MQTT vs Local WebSockets)", "คำถามข้อที่ 2 (MQTT QoS & Features)",
    ...
    var folderName = "Lab 5 Attachments";
    ...
    return {
      status: "success",
      message: "บันทึกข้อมูลใบงานที่ 5 สำเร็จแล้ว! คะแนนประเมินอัตโนมัติ: " + grading.score + "/10.0 คะแนน\n\nรายละเอียดคะแนน:\n" + grading.feedback
    };
