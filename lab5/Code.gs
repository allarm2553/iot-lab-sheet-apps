...
function gradeSubmission(data) {
  ...
  if (challengeCodeText.trim().length > 0) {
    ...
    feedbackDetails.push("- โจทย์ท้าทาย (Challenge Code): ตรงตรรกะ & JSON Serialization " + matchedChallenge + "/" + challengeKeywords.length + " จุดหลัก (+" + challengeScore.toFixed(1) + "/2.5 คะแนน)");
  } else {
...
function submitLabData(data) {
  try {
    ...
    var screenshotUrl = "";
    var codeUrl = "";
    var folderName = "IoT_Lab5_Uploads";
