/**
 * Web App for Lab Extra: Web Dashboard Development
...
function doGet(e) {
  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('ใบงานเสริมปฏิบัติการ: การพัฒนา Web Dashboard')
    .addMetaTag('viewport', 'width=device-width, initial-scale=1')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// Auto-grading logic for Lab Extra
// Auto-grading logic for lab-extra
// Auto-grading logic for lab-extra
function gradeSubmission(data) {
  var blankKeywords = [];
  var challengeKeywords = ["warning-blink|warning|blink", "classList", "add", "remove", "language|lang|th|en"];
  var q1Keywords = ["blur","เบลอ","หลัง","กระจก","โปร่งแสง"];
  var q2Keywords = ["ไอดี","id","อ้างอิง","เข้าถึง","ดึง","element"];
...
function submitLabData(data) {
  try {
    ...
    var sheetName = "Lab Extra Submissions";
    ...
    if (!sheet) {
      ...
      var headers = [
        ...
        "คำอธิบายโค้ด Challenge", "คำถามข้อที่ 1 (backdrop-filter)", "คำถามข้อที่ 2 (document.getElementById)",
        "ลิงก์ไฟล์รูปภาพผลการทดลอง", "ลิงก์ไฟล์โค้ด (.zip)", "สรุปผลการทดลอง",
    ...
    var folderName = "Lab Extra Attachments";
    ...
    return {
      status: "success",
      message: "บันทึกข้อมูลใบงานเสริมสำเร็จแล้ว! คะแนนประเมินอัตโนมัติ: " + grading.score + "/10.0 คะแนน\n\nรายละเอียดคะแนน:\n" + grading.feedback
    };
